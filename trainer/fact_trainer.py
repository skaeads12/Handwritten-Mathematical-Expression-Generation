...
class FactTrainer(BaseTrainer):
    ...
    def train(self, loader, st_step=0, max_step=100000):
        ...
        for batch in cyclize(loader):
            ...
            # style_facts_s: (batch*n_s, n_exp, *features_size)
            # char_facts_c: (batch*n_s, n_exp, *features_size)
            ...
            # mean_style_facts: (batch, n_exp, *features_size)
            # mean_char_facts: (batch, n_exp, *features_size)
            ...
            if self.cfg.gpu <= 0:
                ...
                if self.step % self.cfg.print_freq == 0:
                    ...
                    nrow = len(trg_imgs)
                    grid = utils.make_comparable_grid(
                        style_imgs[:, 0].detach().cpu(),
                        char_imgs[:, 0].detach().cpu(),
                        trg_imgs.detach().cpu(),
                        gen_imgs.detach().cpu(), nrow=nrow
                    )
    ...
    def add_ac_losses_and_update_stats(self, style_facts, style_ids, char_facts, comp_ids,
        ...
        # style_facts: (b * n_s, n_exp, 256, 16, 16)
        # style_ids: (b * n_s, 20)
        # char_facts: (b * n_s, n_exp, 256, 16, 16)
        # comp_ids: (b * n_s, 20)
